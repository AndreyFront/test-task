...
function toggleList(listItem) {
  ...
  if (list) {
    if (!list.classList.contains('hidden')) {
      ...
      list.animate([{ height: list.offsetHeight + 'px' }, { height: 0 }], {
        duration: 400,
        easing: 'linear',
      });
      ...
      listItem
        .querySelector('.marker-list')
        .classList.remove('marker-list--active');
    } else {
      list.classList.remove('hidden');
      list.classList.add('show');

      list.style.display = 'grid';
      list.animate([{ height: 0 }, { height: list.offsetHeight + 'px' }], {
        duration: 400,
        easing: 'linear',
      });
      listItem
        .querySelector('.marker-list')
        .classList.add('marker-list--active');
    }
  }
}
...
function updateLocalStorage() {
  if (
    localStorage.getItem('favoritesImage') != '' &&
    localStorage.getItem('favoritesImage') != undefined
  )
  ...
  if (event.target.closest('.favorites-icon')) {
    ...
    try {
      if (!favoritesIcon.classList.contains('favorites-icon--active')) {
        ...
        arrayImages = JSON.parse(localStorage.getItem('favoritesImage'));
        arrayImages.splice(
          arrayImages.findIndex((el) => el.id === cardImageId),
          1
        );
        localStorage.setItem('favoritesImage', JSON.stringify(arrayImages));
      }
      ...
      if (arrayImages !== undefined && arrayImages !== null) {
        if (arrayImages.length > 0) {
          arrayImages.forEach((image) => {
            content.querySelector('ul').insertAdjacentHTML(
              'beforeEnd',
              `
        ...
        .then(() => {
          content.addEventListener('click', (event) => {
            if (event.target.closest('.list__block-item')) {
              ...
              try {
                if (
                  !listItem.parentNode.hasAttribute('data-available') &&
                  listItem.parentNode.hasAttribute('data-user-id')
                ) {
                  listItem.parentNode.setAttribute('data-available', '');
                  let parentListItem = Number(
                    listItem.parentNode.getAttribute('data-user-id')
                  );
                  sendRequest('GET', `${requestAlbums + parentListItem}`)
                    .then((dataAlbums) => {
                      if (
                        arrayUsers.indexOf(dataAlbums[0].userId) !== -1 &&
                        parentListItem === dataAlbums[0].userId
                      ) {
                        const listItem = document.querySelector(
                          `.list__item[data-user-id="${dataAlbums[0].userId}"]`
                        );
                        ...
                        dataAlbums.forEach((album) => {
                          ...
                          listItem.querySelector('ul').insertAdjacentHTML(
                            'beforeEnd',
                            `
                ...
                if (
                  !listItem.parentNode.hasAttribute('data-available') &&
                  listItem.parentNode.hasAttribute('data-album-id')
                ) {
                  listItem.parentNode.setAttribute('data-available', '');
                  let parentListItem = event.target
                    .closest('.list__item[data-album-id]')
                    .getAttribute('data-album-id');
                  sendRequest('GET', `${requestImage + parentListItem}`)
                    .then((dataImage) => {
                      if (
                        arrayAlbums.indexOf(dataImage[0].albumId) !== -1 &&
                        +parentListItem === dataImage[0].albumId
                      ) {
                        let listItem = document.querySelector(
                          `.list__item[data-album-id="${dataImage[0].albumId}"]`
                        );
                        listItem.insertAdjacentHTML(
                          'beforeEnd',
                          `
                        ...
                        dataImage.forEach((image) => {
                          ...
                          if (
                            ...
                            if (
                              arrayImages.findIndex(
                                (el) => el.id == image.id
                              ) != -1
                            )
                              activeIcon = 'favorites-icon--active';
                          }
                          listItem.querySelector('ul').insertAdjacentHTML(
                            'beforeEnd',
                            `
